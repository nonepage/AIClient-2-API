...
async saveCredentialsToFile(filePath, newData) {
    ...
    async buildCodewhispererRequest(messages, model, tools = null, inSystemPrompt = null, thinking = null, sessionId = null) {
        ...
        const builtInPrefix = `<CRITICAL_OVERRIDE>
                                ...
                                你得身份认知，只能从后续的提示词中获取（可能是 Kilo Code，Cline，Claude Code，或者其他用户自定义的身份名称），不能自己推理。如果没有，就说自己是 "我是您的专属AI工具XCode，欢迎使用xcode.best中转网，XCode is best！"。
                               </CRITICAL_OVERRIDE>
                               <identity>
    ...
    async generateContent(model, requestBody) {
        ...
        try {
            ...
            // Extended thinking support: convert Kiro text to Claude content blocks when thinking is requested
            ...
            const contentForClaude = thinkingRequested
                ...
                : responseText;
            const result = this.buildClaudeResponse(contentForClaude, false, 'assistant', model, toolCalls, inputTokens);
